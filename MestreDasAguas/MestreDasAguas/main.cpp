...
int main() {
    ...
    // Música do Menu
    Music musicaMenu;
    bool musicaMenuCarregada = false;

    // Música do Jogo
    Music musicaJogo;
    bool musicaJogoCarregada = false;
    ...
    for (const auto& caminho : caminhosMusicais) {
        if (!musicaMenuCarregada && musicaMenu.openFromFile(caminho + "musica.mp3")) {
            musicaMenuCarregada = true;
            musicaMenu.setLooping(true);
            musicaMenu.setVolume(20);
            break;
        }
    }
    ...
    for (const auto& caminho : caminhosMusicais) {
        if (!musicaJogoCarregada && musicaJogo.openFromFile(caminho + "game.mp3")) {
            musicaJogoCarregada = true;
            musicaJogo.setLooping(true);
            musicaJogo.setVolume(20);
            break;
        }
    }
    ...
    while (window.isOpen()) {
        ...
        // Verificar mudança de fase para trocar música
        if (fase != faseAnterior) {
            if (fase == COLOCACAO_NAVIOS && faseAnterior == MENU) {
                // Saiu do menu para colocação - trocar para música de jogo
                musicaMenu.stop();
                if (musicaJogoCarregada && musicaAtiva) {
                    musicaJogo.play();
                }
            }
            else if (fase == JOGANDO && faseAnterior == COLOCACAO_NAVIOS) {
                // Mudou para fase de jogo - continua música de jogo (não faz nada)
            }
            else if (fase == MENU) {
                // Voltou para menu - trocar para música de menu
                musicaJogo.stop();
                if (musicaMenuCarregada && musicaAtiva) {
                    musicaMenu.play();
                }
            }
            else if (fase == FIM) {
                // Fim de jogo - continuar com música de jogo (não faz nada)
            }
            faseAnterior = fase;
        }
        ...
        while (auto event = window.pollEvent()) {
            ...
            if (event->is<Event::MouseButtonPressed>()) {
                ...
                if (mouseEvent && mouseEvent->button == Mouse::Button::Left && botaoMusica.foiClicado()) {
                    ...
                    musicaAtiva = botaoMusica.ativo;
                    sons.ativo = botaoMusica.ativo;

                    // Controlar música baseado na fase atual
                    if (musicaAtiva) {
                        if (fase == JOGANDO || fase == COLOCACAO_NAVIOS) {
                            if (musicaJogoCarregada) {
                                musicaJogo.play();
                            }
                        }
                        else if (fase == MENU) {
                ...
                if (event->is<Event::MouseButtonPressed>()) {
                    ...
                    if (mouseEvent && mouseEvent->button == Mouse::Button::Left) {
                        if (botaoJogarNovamente.foiClicado(mousePos)) {
                            ...
                            // Iniciar música de jogo se música estiver ativa
                            if (musicaAtiva && musicaJogoCarregada) {
                                musicaJogo.play();
                            }
                        }
        ...
        if (fase == MENU) {
            ...
            // Desenhar mensagem acima do tabuleiro
            desenharTexto(window, mensagem, 80, 45, 16, Color::Yellow, font);
